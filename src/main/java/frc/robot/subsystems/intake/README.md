...
**Spinning wheels are pretty similar**. This is a simple flywheel subsystem; it could probably be used as the basis for e.g. a shooter as well. The key is velocity-based tuning.
...
**Consider brakes**. Intakes tend to roll slowly, and the emphasis is on control and positioning of a gamepiece. So having the motor brake enabled is useful - when the motor stops turning, the wheels will essentially freeze in place. For a shooter, which usually moves much faster, immediately stopping might stress or even worse, break the wheels.
