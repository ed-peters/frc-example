...
**There are many ways to drive to an offset**. We switched between using the WPILib trajectory support, and a very basic PID-based approach. This implementation strikes a balance and uses a `TrapezoidProfile` to drive along a straight line to a target position. See `SwerveTargetPoseOffsetCommand`. 